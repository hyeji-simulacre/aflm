...
저장하면 GitHub에 커밋되고 Vercel이 사이트를 다시 만든다. 재보니 15~30초였다. 커밋이 그대로 변경 이력이자 백업이므로 별도 데이터베이스를 두지 않는다.

기다리는 동안 무엇이 저장됐는지 알 수 없으면 곤란하므로, 저장한 값을 입력 화면에 그대로 보여준다. 그 아래 '사이트에서 보기' 링크는 공개 목록 데이터를 4초마다 다시 받아 저장한 값이 나타나면 열린다. 3분 안에 확인되지 않으면 확인하지 못했다고 적는다. 저장이 끝난 것과 사이트에 나타난 것을 구분해서 알린다.

기록이 사이트에 나타나기까지 다시 만드는 과정이 필요한 이유는 목록 데이터가 빌드 산출물이기 때문이다. `public/data/movies.json`은 보여줄 값만 담은 요약본이고, 카드가 차지할 격자 행 수를 첫 대사 길이로 계산해 넣는다. 편별 상세 페이지도 편마다 정적 파일 하나다.
